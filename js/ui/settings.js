...
const BUBBLE_PRESETS = [
  ...
  },
  {
    id: 'wechat',
    name: '微信',
    css: `.message-bubble.sent { background: #95ec69 !important; color: #000 !important; border-radius: 18px !important; border-bottom-right-radius: 4px !important; }
.message-bubble.received { background: #fff !important; color: #333 !important; border-radius: 18px !important; border-bottom-left-radius: 4px !important; }`,
    previewSent: '#95ec69',
    previewReceived: '#fff'
  },
  {
    id: 'dark',
    name: '暗黑',
    css: `.message-bubble.sent { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%) !important; color: #fff !important; border-radius: 18px !important; border-bottom-right-radius: 4px !important; }
.message-bubble.received { background: rgba(255,255,255,0.1) !important; color: #e0e0e0 !important; border: 1px solid rgba(255,255,255,0.15) !important; border-radius: 18px !important; border-bottom-left-radius: 4px !important; }`,
    previewSent: '#764ba2',
    previewReceived: '#444'
  },
  {
    id: 'minimal',
    name: '极简',
    css: `.message-bubble.sent { background: #f0f0f0 !important; color: #333 !important; border-radius: 12px !important; box-shadow: none !important; }
.message-bubble.received { background: #fff !important; color: #333 !important; border-radius: 12px !important; border: 1px solid #eee !important; box-shadow: none !important; }`,
    previewSent: '#f0f0f0',
    previewReceived: '#fff'
  },
];
...
function bindEvents() {
  ...
  resetCss?.addEventListener('click', () => {
    ...
    if (c) {
      ...
      updateBubbleCssPreview(previewBox, '', true, colorThemes['white_pink']);
  ...
  // █ AI 评估态度按钮
  const evalAttBtn = dom['btn-evaluate-attitude'];
  if (evalAttBtn && !evalAttBtn._bound) {
    evalAttBtn._bound = true;
    evalAttBtn.addEventListener('click', async () => {
      const c = getDb().characters.find(ch => ch.id === state.currentChatId);
      if (!c || !c.history || c.history.length === 0) {
        showToast(dom['toast-notification'], '没有聊天记录可以评估');
        return;
      }
      const db = getDb();
      const api = db.apiPresets?.find(p => p.id === db.activeApiPresetId) || db.apiSettings || {};
      ...
      const originalText = evalAttBtn.textContent;
      evalAttBtn.textContent = '⏳ 评估中...';
      evalAttBtn.disabled = true;
      try {
        // 取最近 40 条有效对话
        const historyText = c.history.map(m => {
          const t = (m.content || '').replace(/\[.*?\]/g, '').trim();
        ...
        const prompt = `请根据以下聊天记录，分析角色「${c.remarkName}」对「我」的态度。要求：
1. 用 1-2 句简洁中文描述
2. 指出态度是冷淡/热情/依赖/抗拒/暧昧等
3. 包含这种态度在最近对话中的表现
格式要求：直接输出态度描述，不要前缀和引号。
...
${historyText}`;
        ...
        if (api.provider === 'gemini') {
          ...
          const resp = await fetch(apiUrl + '/v1/chat/completions', {
            method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + api.key },
            body: JSON.stringify({ model: api.model, stream: false, temperature: 0.5, max_tokens: 200, messages: [{ role: 'user', content: prompt }] })
          });
          const json = await resp.json();
          fullText = (json.choices && json.choices[0] && json.choices[0].message) ? json.choices[0].message.content : '';
        }
        c.attitude = fullText.trim() || '(评估失败)';
        dom['setting-char-attitude'].value = c.attitude;
        await saveData();
        showToast(dom['toast-notification'], '✅ 态度评估完成');
      } catch (err) {
        console.error('[Attitude Eval]', err);
        showToast(dom['toast-notification'], '评估失败: ' + err.message);
      } finally {
        evalAttBtn.textContent = originalText;
        evalAttBtn.disabled = false;
      }
    });
  }
...
function loadSettingsToSidebar() {
  ...
  // ★ 态度编辑栏
  if (dom['setting-char-attitude']) dom['setting-char-attitude'].value = c.attitude || '';
