...
const aiChat = window.AiService?.chat;
const generateImage = window.AiService?.generateImage;
...
function bindEvents() {
  ...
  dom['message-area']?.addEventListener('click', (e) => {
    ...
    const transferClick = e.target.closest('.transfer-card.received-transfer');
    if (transferClick) {
      const db = dataService.getDb();
      const msg = (db.characters || []).find(c => c.id === state.currentChatId)?.history.find(m => m.id === transferClick.dataset.messageId);
      if (msg && msg.transferStatus === 'pending') handleReceivedTransferClick(transferClick.dataset.messageId);
    }
    // 礼物卡片点击展开/收起描述
    const gc = e.target.closest('.gift-card');
    if (gc) {
      const desc = gc.closest('.message-wrapper')?.querySelector('.gift-card-description');
      if (desc) desc.classList.toggle('active');
    }
    // 语音气泡点击展开/收起文字稿
    const vb = e.target.closest('.voice-bubble');
    if (vb) {
      const tr = vb.closest('.message-wrapper')?.querySelector('.voice-transcript');
      if (tr) tr.classList.toggle('active');
    }
    // 照片/视频卡片点击切换覆盖层
    const pv = e.target.closest('.pv-card');
    if (pv) {
      pv.querySelector('.pv-card-image-overlay')?.classList.toggle('hidden');
      pv.querySelector('.pv-card-footer')?.classList.toggle('hidden');
    }
...
export function createMessageBubbleElement(message) {
  ...
  const hasPendingImage = message.pendingImagePrompt && !hasImage;
  ...
  if (hasImage) {
    ...
    placeholder.style.cssText = 'width:200px;height:150px;background:#f5f5f5;border-radius:12px;display:flex;align-items:center;justify-content:center;color:#999;font-size:13px;';
    placeholder.innerHTML = '<div style="text-align:center;"><div style="font-size:28px;margin-bottom:4px;">🎨</div><div>正在生成配图...</div></div>';
    ...
    // 检测表情包
    const sentStickerMatch = message.content.match(/\[(?:.+?)的表情包：(.+?)\]/i);
    const receivedStickerMatch = message.content.match(/\[(?:.+?)发送的表情包：([\s\S]+?)\]/i);
    if ((isSent && sentStickerMatch) || (!isSent && receivedStickerMatch)) {
      const rawPath = isSent ? sentStickerMatch[1].trim() : receivedStickerMatch[1].trim();
      // 本地路径直接使用，远程URL也直接使用，其他才拼接postimg前缀
      const stickerSrc = rawPath.startsWith('http') || rawPath.startsWith('assets/')
        ? rawPath
        : `https://i.postimg.cc/${rawPath.match(/[a-zA-Z0-9]+\/(.*$)/)?.[0] || rawPath}`;
      bubble.className = 'image-bubble';
      bubble.innerHTML = `<img src="${stickerSrc}" alt="表情包" style="max-width:180px;max-height:180px;border-radius:8px;">`;
    }
    // 检测语音
    else if (/\[(?:.+?)的语音：([\s\S]+?)\]/.test(message.content)) {
      const voiceMatch = message.content.match(/\[(?:.+?)的语音：([\s\S]+?)\]/);
      const voiceText = voiceMatch ? voiceMatch[1].trim() : '';
      const duration = utils.calculateVoiceDuration(voiceText);
      ...
      bubble.innerHTML = `<svg class="play-icon" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"></path></svg><span class="duration">${duration}"</span>`;
      const trDiv = document.createElement('div');
      trDiv.className = 'voice-transcript';
      trDiv.textContent = voiceText;
      wrapper.appendChild(trDiv);
    }
    // 检测照片/视频
    else if (/\[(?:.+?)发来的照片\/[\s\S]*?视频：([\s\S]+?)\]/.test(message.content)) {
      const pvMatch = message.content.match(/\[(?:.+?)发来的照片\/[\s\S]*?视频：([\s\S]+?)\]/);
      const pvText = pvMatch ? pvMatch[1].trim() : '';
      bubble.className = 'pv-card';
      const bgImg = isSent ? 'https://i.postimg.cc/L8NFrBrW/1752307494497.jpg' : 'https://i.postimg.cc/1tH6ds9g/1752301200490.jpg';
      bubble.innerHTML = `<div class="pv-card-content">${pvText}</div><div class="pv-card-image-overlay" style="background-image: url('${bgImg}');"></div><div class="pv-card-footer"><svg viewBox="0 0 24 24"><path d="M4,4H20A2,2 0 0,1 22,6V18A2,2 0 0,1 20,20H4A2,2 0 0,1 2,18V6A2,2 0 0,1 4,4M4,6V18H20V6H4M10,9A1,1 0 0,1 11,10A1,1 0 0,1 10,11A1,1 0 0,1 9,10A1,1 0 0,1 10,9M8,17L11,13L13,15L17,10L20,14V17H8Z"></path></svg><span>照片/视频・点击查看</span></div>`;
    }
    // 检测礼物
    else if (message.giftStatus) {
      const giftMatch = message.content.match(/\[(?:.+?)送来的礼物：([\s\S]+?)\]/);
      const giftDesc = giftMatch ? giftMatch[1].trim() : '礼物';
      bubble.className = 'gift-card';
      if (message.giftStatus === 'received') bubble.classList.add('received');
      bubble.innerHTML = `<img src="assets/icons/礼物.jpg" alt="gift" class="gift-card-icon"><div class="gift-card-text">您有一份礼物～</div><div class="gift-card-received-stamp">已查收</div>`;
      const descDiv = document.createElement('div');
      descDiv.className = 'gift-card-description';
      descDiv.textContent = giftDesc;
      wrapper.appendChild(descDiv);
      bubble.dataset.messageId = message.id;
    }
    // 检测转账
    else if (message.transferStatus) {
      const transferMatch = message.content.match(/\[.*?的转账：([\d.]+)元；备注：([^\]]*)\]/);
      const amount = transferMatch ? parseFloat(transferMatch[1]).toFixed(2) : '0.00';
      const remark = transferMatch ? transferMatch[2] : '';
      const isSentTransfer = isSent;
      const cardType = isSentTransfer ? 'sent-transfer' : 'received-transfer';
      bubble.className = `transfer-card ${cardType}`;
      if (message.transferStatus === 'received') bubble.classList.add('received');
      else if (message.transferStatus === 'returned') bubble.classList.add('returned');
      let statusText = isSentTransfer ? '待查收' : '转账给你';
      if (message.transferStatus === 'received') statusText = '已收款';
      else if (message.transferStatus === 'returned') statusText = '已退回';
      if (message.transferStatus !== 'pending') bubble.style.cursor = 'default';
      bubble.dataset.messageId = message.id;
      bubble.innerHTML = `<div class="overlay"></div><div class="transfer-content"><p class="transfer-title">${isSentTransfer ? '转账' : '转账给你'}</p><p class="transfer-amount">¥${amount}</p>${remark ? `<p class="transfer-remark">${remark}</p>` : ''}<p class="transfer-status">${statusText}</p></div>`;
    ...
    // 转账卡片使用自己的背景图，不覆盖
    if (!bubble.classList.contains('transfer-card')) {
      bubble.style.backgroundColor = isSent ? bubbleTheme.sent.bg : bubbleTheme.received.bg;
      bubble.style.color = isSent ? bubbleTheme.sent.text : bubbleTheme.received.text;
    }
  } else {
    ...
    speakBtn.addEventListener('click', (e) => {
      ...
      import('../systems/ttsService.js').then(tts => {
        if (tts.isSpeaking()) {
          ...
          // 获取角色私有 TTS 配置，回退到全局配置
          const db = dataService.getDb();
          const c = state.currentChatType === 'private'
            ? dataService.getCharacter(state.currentChatId)
            : dataService.getGroup(state.currentChatId);
          const charCfg = c?.ttsConfig;
          const globalCfg = db.ttsConfig || {};
          const ttsOpts = {};
          if (charCfg && charCfg.engine !== 'global') {
            ttsOpts.engine = charCfg.engine;
            ttsOpts.sogouSpeaker = charCfg.sogouSpeaker;
            ttsOpts.sogouSpeed = charCfg.sogouSpeed;
          } else {
            ttsOpts.engine = globalCfg.engine;
            ttsOpts.sogouSpeaker = globalCfg.sogouSpeaker;
...
export async function sendSticker(sticker) {
  ...
  // 区分本地路径和远程URL
  const stickerPath = sticker.data.startsWith('assets/')
    ? sticker.data
    : sticker.data.replace('https://i.postimg.cc/', '');
  const msg = {
    id: 'msg_' + Date.now() + '_' + Math.random(),
    role: 'user',
...
export async function sendMyGift(item) {
  ...
  // 从库存扣除（消耗型才删除，装备型保留）
  if (db.ownedItems && item.consumable) {
    db.ownedItems = db.ownedItems.filter(id => id !== item.id);
  }
  utils.showToast(dom['toast-notification'], `已送出 ${item.label || '礼物'}`);
  // 送礼撒花特效
  if (typeof confetti === 'function') {
    confetti({ particleCount: 45, spread: 60, origin: { y: 0.7 }, colors: ['#667eea','#9b59b6','#f093fb','#a5b4fc'], gravity: 0.9 });
  }
  await dataService.saveData();
  if (_renderChatList) _renderChatList();
  triggerAiReply();
...
export function updateTransferBalanceDisplay() {
  const db = dataService.getDb();
  const balanceEls = document.querySelectorAll('.wallet-balance, #wallet-balance, #transfer-balance');
...
async function handleAiResponse(fullResponse, chat) {
  // 检测 AI 自主决定的配图标记
  const imgRegexGlobal = /\[(?:生成配图|配图|生成图片|(?:[^\]]+?)(?:的照片|(?:发来|发送)(?:的照片)?(?:[\/／]视频)?|的自拍))(?:[：:]\s*([^\]]+))?\]/g;
  let imgPrompts = [];
  let imgMatch;
  while ((imgMatch = imgRegexGlobal.exec(fullResponse)) !== null) {
    imgPrompts.push(imgMatch[1].trim());
  }
  ...
  if (state.currentChatType === 'private') {
    ...
    if (messages.length > 0) {
      for (const item of messages) {
        ...
        if (imgPrompts.length > 0) message.pendingImagePrompt = imgPrompts.shift();
...
async function generateImageForMessage(msgId, prompt, chat) {
  ...
  try {
    const imageUrl = await generateImage(prompt + ', anime style, high quality', { imageSize: '768x1024' });
    ...
    placeholder.outerHTML = `<img src="${persistentUrl}" alt="AI配图" class="ai-generated-image" style="max-width:100%;max-height:300px;border-radius:8px;margin-top:6px;cursor:pointer;">`;
    const msg = (chat.history || []).find(m => m.id === msgId);
    if (msg) {
      msg.imageUrl = persistentUrl;
      msg.pendingImagePrompt = undefined;
      msg._imageGenStarted = undefined;
      msg.type = 'ai_image';
      if (!msg.parts) msg.parts = [];
      if (!msg.parts.some(p => p.type === 'image')) msg.parts.push({ type: 'image', data: persistentUrl });
    }
    await dataService.saveData();
  } catch (err) {
    console.error('[AiImg] 生成失败:', err);
    if (placeholder) {
      placeholder.innerHTML = '<span style="color:#999;font-size:12px;">🖼️ 图片生成失败</span>';
      placeholder.classList.add('image-placeholder-failed');
    }
  }
}
...
export async function maybeSendAiImage(chat, prompt) {
...
function fallbackCopy(text) {
  ...
  ta.select();
  try { document.execCommand('copy'); utils.showToast(dom['toast-notification'], '已复制到剪贴板'); }
  catch(e) { utils.showToast(dom['toast-notification'], '复制失败'); }
...
function handleMessageLongPress(wrapper, x, y) {
  ...
  items.push({ label: '📋 复制', action: () => {
    const text = msgContent.replace(/\[.*?\]/g, '').trim();
    if (navigator.clipboard && text) {
      navigator.clipboard.writeText(text).then(() => {
        utils.showToast(dom['toast-notification'], '已复制到剪贴板');
      }).catch(() => fallbackCopy(text));
    } else if (text) {
      fallbackCopy(text);
    } else {
      utils.showToast(dom['toast-notification'], '没有可复制的内容');
    }
  }});
  if (isSent) {
