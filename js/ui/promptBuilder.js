...
export function generatePrivateSystemPrompt(character) {
  ...
  if (character.aiImgGen) {
    p += `17. ✨重要✨ 你可以生成配图。当你想在回复中配一张图片时，在普通消息中插入 [生成配图：对画面的详细描述] 或 [配图：描述]。也可以用 [${character.realName}发来的照片/视频：描述] 或 [${character.realName}的照片：描述] 或 [${character.realName}的自拍：描述] 来触发配图。这些标记不会显示给用户，会触发系统生成对应图片发送到聊天中。只有当你觉得配图能让对话更生动时再使用，不要每条消息都配图。`;
  }
  return p;
}
...
export function generateGroupSystemPrompt(group) {
  ...
  if ((group.members || []).some(m => m.aiImgGen) || group.aiImgGen) {
    p += `8. 你有生成配图的能力。当你想在回复中配图时，在对应成员消息中插入 [生成配图：描述] 或 [配图：描述]。也可以用 [{成员真名}发来的照片/视频：描述]、[{成员真名}的照片：描述] 或 [{成员真名}的自拍：描述] 来触发配图。这些标记不会显示给用户，会触发系统生成对应图片。不要每条消息都配图，只在合适时使用。`;
  }
  return p;
}
