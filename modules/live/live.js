...
Engine.register({
  ...
  _prp(b){
    ...
    var items=shop?shop.getLiveProps():[];
    ...
    b.innerHTML=
      ...
      '<div class="live-wizard-body">'+
        (items.length===0?'<div class="live-empty-state">暂无可选道具<br><small>需要先在商店购买直播装备</small></div>':
          '<div class="live-prop-grid">'+items.map(function(p){
            var icon=p.icon||'';
            var n=p.label;
            var s=(this.state.props||[]).indexOf(p.id)>=0;
            return '<div class="live-prop-item'+(s?' selected':'')+'" onclick="Engine._lv._tglProp(\''+p.id+'\',this)">'+icon+' '+n+'</div>';
          },this).join('')+'</div>')+
      '</div>'+
  ...
  async start(){
    ...
    if(shop&&this.state.props.length>0){
      ...
      this.state.props.slice(0,3).forEach(function(pid){
        ...
        if(p) tags.innerHTML+='<span class="live-tag">'+(p.icon||'')+' '+p.label+'</span>';
      });
    }
  ...
  /* ─── AI生成 ─── */
  async gen(uin){
    if(this._isGenBusy)return; this._isGenBusy=true;
    var lv=this.state.level, sc=this.state.scene;
    ...
    if(this.state.props.length>0){
      var shop=Engine.getModule('shop');
      var all=shop?shop.getAllItems():[];
      var propNames=this.state.props.map(function(pid){
        var p=all.find(function(d){return d.id===pid;});
        return p?(p.icon||'')+p.label:'';
      }).filter(Boolean);
      if(propNames.length>0) propsInfo=' 直播使用了以下道具：'+propNames.join('、')+'。请在剧情中自然体现这些道具的使用效果。';
    }

    var sys='你是成人向直播内容生成器。虚构内容，成年人。等级：'+lv.label+' 场景：'+sc.label+' '+hi+' 尺度：'+ls+' '+persp+propsInfo+'\n返回JSON数组：{"type":"类型","text":"内容"}。type: story(剧情粉字),danmaku(弹幕白字,text格式"昵称: 内容"),tip(打赏金字),gift(礼物金字加粗)。15-25条混合输出。剧情要详细，每条story至少2-3句话有画面感和互动细节。先推进剧情再混入弹幕。只返回JSON数组。';
    ...
    try{
      ...
      data.forEach(function(d){
        if(d.type==='danmaku'){
          ...
          h._msg(d.type,d.text);
          if(d.type==='story') h.stats.story++;
          else if(d.type==='tip'){
            var m=d.text.match(/(\d+)/);
            if(m){ var amt=parseInt(m[1]); h.tips+=amt; h.stats.totalTips+=amt; }
          }
        }
      });
    }catch(e){
  ...
  async genBg(){
    ...
    var lv=this.state.level, sc=this.state.scene;
    var hn=this.host?(this.host.remarkName||this.host.realName):'';
    try{
      var brief=(hn?hn+'，':'')+sc.label.replace(/^\S+\s/,'')+'，'+lv.label;
      var url=await Engine.services.aiGenerateImage('live background, '+brief+', anime style',{imageSize:'768x1024'});
  ...
  /* ─── 结束 ─── */
    ...
    // 道具加成：每件 +5%，上限 30%
    var propCount=(this.state.props||[]).length;
    var propBonus=Math.min(propCount*0.05,0.30);
    var baseCoins=(this.stats.story*10+this.stats.dms*2+this.tips)*(this.LEVEL_MULTIPLIER[this.state.level.id]||1);
    var coins=Math.floor(baseCoins*(1+propBonus));
    db.money=(db.money||0)+coins; saveData();

    var b=document.getElementById('lvsb');
    var bonusText=propCount>0?'<div style="font-size:12px;color:rgba(255,255,255,0.5);margin-top:4px;">🎬 道具加成: +'+Math.round(propBonus*100)+'% ('+propCount+'件)</div>':'';
    b.innerHTML=
      '<div class="live-settle-title">🎉 直播结束</div><div class="live-settle-sub">精彩！</div>'+
      '<div class="live-settle-coin">+'+coins+' 🪙</div>'+bonusText+
